...
class Muster13PreviewFrame(ttk.Frame):
    ...
    def _update_codelisten_hint(self):
        ...
        # Alle importierten Zusatzquellen mit ihrem Umfang und Stand. Fehlt
        # eine, wird sie nicht erwähnt — nur die Heilmittelpreise sind so
        # zentral, dass ihr Fehlen als Hinweis erscheint.
        zeilen.extend(codelisten.quellen_beschreibung())
    ...
    def _fill_versicherter(self, beleg: Dict[str, Any]):
        ...
        # describe_ik setzt den Kassennamen dahinter; steht der IK in keinem
        # Verzeichnis, sagt es das ausdrücklich, statt die Zahl allein zu zeigen.
        self._set("ik", codelisten.describe_ik(beleg.get("kostentraeger_ik"), leer_text=""))
        self._set("ik_kasse", codelisten.describe_ik(beleg.get("krankenkasse_ik"), leer_text=""))
    ...
    def _fill_verordnung(self, beleg: Dict[str, Any]):
        ...
        # Diagnosen mit Verordnungsbedarf: die KBV-Stammdatei Heilmittelanlagen
        # sagt, ob der ICD-Code in Anlage 2 (Langfristiger Heilmittelbedarf)
        # oder Anlage 3 (Besonderer Verordnungsbedarf) steht. Genau das erklärt
        # in der Hotline, warum eine Verordnung Besonderheiten trägt.
        diagnosen = beleg.get("diagnosen") or []
        dia_zeilen = []
        for d in diagnosen:
            code = str(d.get("code", ""))
            zeile = code + (f"  —  {d['text']}" if d.get("text") else "")
            bedarf = codelisten.verordnungsbedarf(code)
            if bedarf["sicherheit"]:
                zeile += f"   (Diagnosesicherheit {bedarf['sicherheit']})"
            dia_zeilen.append(zeile)
            if bedarf["text"]:
                dia_zeilen.append(f"        {bedarf['text']}")
                for eintrag in bedarf["treffer"]:
                    if eintrag.get("hinweis"):
                        dia_zeilen.append(f"        {eintrag['hinweis']}")

        self._set("vo_diagnosen", "\n".join(dia_zeilen) or "keine Diagnose im Beleg (DIA fehlt)")
