...
class Muster13PreviewFrame(ttk.Frame):
    ...
    def __init__(self, parent: tk.Widget):
        ...
        # Pfade zu den Grafikvorlagen (unterstützt auch gepackte PyInstaller EXE via sys._MEIPASS)
        meipass = getattr(sys, "_MEIPASS", None)
        self.base_dir = meipass if meipass else os.path.dirname(os.path.abspath(__file__))
    ...
    def _setup_ui(self):
        ...
        # Zeile 2: Status Codelisten & HMP-Klartexte
        self.lbl_codelisten = ttk.Label(
            sum_frame, text="", font=("Segoe UI", 8), foreground="#666666"
        )
        self.lbl_codelisten.grid(row=2, column=0, columnspan=8, sticky="w", **pad)
    ...
    def _update_codelisten_hint(self):
        """Aktualisiert die Hinweiszeile über geladene Codelisten und HMP-Preise."""
        ...
        if codelisten.hmp_source_path():
            ...
            zeilen.append("Keine Heilmittelpreis-Bezeichnungen (tools/import_hmp.py ausführen)")
    ...
    def apply_theme(self, mode: Optional[str] = None):
        ...
        if active_mode == "dark":
            ...
            self.lbl_codelisten.config(foreground="#888888")
            ...
            self.lbl_codelisten.config(foreground="#666666")
    ...
    def load_beleg(self, beleg: Dict[str, Any], validation_errors: Optional[List[str]] = None):
        ...
        zkz_text = ""
        if isinstance(zhe, dict) and zhe.get("zuzahlungskennzeichen_text"):
            zkz_text = str(zhe["zuzahlungskennzeichen_text"])
        if not zkz_text:
            zkz_text = codelisten.describe("zuzahlungskennzeichen", zkz, leer_text="")
        if not zkz_text:
            zkz_labels = {
                "0": "0 — Keine Zuzahlung",
            ...
            zkz_text = zkz_labels.get(zkz, f"Kennzeichen {zkz}")
        self.lbl_zuz_status.config(text=zkz_text)
    ...
    def _render_front_side(
        ...
        zhe = beleg.get("verordnung") or {}
        if not hm_bereich and isinstance(zhe, dict):
            hm_bereich = str(zhe.get("heilmittelbereich", ""))
        ...
        def _resolve_pos_label(pos_dict: Dict[str, Any]) -> str:
            code = str(pos_dict.get("code", "")).strip()
            if code in code_labels:
                return code_labels[code]
            if pos_dict.get("code_klartext"):
                return str(pos_dict["code_klartext"])
            abr = pos_dict.get("abr_code") or beleg.get("abrechnungscode")
            info = codelisten.position_info(code, abr)
            if info.get("bezeichnung"):
                return info["bezeichnung"]
            return f"Heilmittel Pos. {code}" if code else f"Behandlung ({pos_dict.get('tag', 'EHE')})"
        ...
        if ergaenzend_pos:
            ...
            label_erg = _resolve_pos_label(pos_erg)
